  ...
  useEffect(() => {
    ...
    const handleStorageChange = (event: StorageEvent) => {
      ...
      if (
        ...
        if (
          (event.key === "auth_token" && event.newValue === null) ||
          ...
          if (isAuthenticated && !logoutTriggered.current) {
            ...
            alert(
              "Phiên đăng nhập của bạn đã kết thúc ở tab khác. Vui lòng đăng nhập lại."
            );
    ...
    const initializeAuth = async () => {
      try {
        ...
        if (token && savedUser && wasLoggedIn) {
          ...
          console.log(
            "Token hợp lệ nhưng thiếu thông tin user, thử lấy thông tin từ API"
          );
